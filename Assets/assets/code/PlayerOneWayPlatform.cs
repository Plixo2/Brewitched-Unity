...
namespace assets.code
{
    public class PlayerOneWayPlatform : MonoBehaviour
    {
        ...
        private readonly HashSet<GameObject> currentOneWayPlatforms = new();
        ...
        private IEnumerator DisableCollision()
        {
            ...
            foreach (var gameObject in currentOneWayPlatforms)
                plattforms.Add(gameObject.GetComponent<Collider2D>());

            foreach (var plattform in plattforms) Physics2D.IgnoreCollision(playerCollider, plattform);

            yield return new WaitForSeconds(disableCollisionTime);
            foreach (var plattform in plattforms) Physics2D.IgnoreCollision(playerCollider, plattform, false);
        }
    }
}